...
public class Arvbin<T extends Comparable<T>>
{
	...
	/* Construtor de árvore sem sub-árvores (dir = esq = null). Fornecido apenas o valor da raiz. */
	...
	/* Construtor de árvore com sub-árvores. São fornecidos o valor da raiz
	 * e as subárvores, que devem ter sido construídas previamente.*/
	...
	/* Retorna a subárvore esquerda (ou null se não houver). */
	public Arvbin<T> retornaEsq()
	{
		return esq;
	}

	/* Retorna a subárvore direita (ou null se não houver). */
	...
	public Arvbin<T> busca(T valor) // Complexidade O(n)
	{
		Arvbin<T> ret;
	 ...
	 * Calcular o comprimento do maior caminho na árvore, do nó mais a esquerda até o mais a direita, pode ou não passar pela raiz*/
	public int calculaDiametro() // Complexidade O(n^2)
	{
	...
	{
		//Deque é uma fila dupla, posso inserir/retirar no fim e no começo
		...
		/* Caso geral: deve-se fazer a recursão para os nós esquerdo 
		 * (se houver) e para o direito (se houver). */
		...
		/* Remove o nó corrente do caminho apó as chamadas recursivas para os
		 * nós esquerdo e direito. */ 
		caminhos.removeLast();
		
		/* Tipos primitivos -> passagem por valor, o método recebe o valor da variável.
		 * Tipos complexos -> passagem por referência, o método recebe o endereço da variável. */
	}
